...
exports.mineRock = onCall(async (request) => {
  ...
  const lastRockDate =
    typeof lastRockDateRaw === "string" ? lastRockDateRaw : null;
  ...
  const materialsMatch = MATERIAL_KEYS.every(
    (key) => delta[key] === expectedDelta[key],
  );
  if (!materialsMatch) {
    return {
      ok: false,
  ...
  const currentMaterials = normalizeMaterials(
