...
const exchangeXCodeForAccessToken = async ({
  code,
  callbackUri,
  codeVerifier,
}) => {
  ...
  if (!tokenResponse.ok) {
    const providerError = toCleanString(
      tokenPayload && (tokenPayload.error_description || tokenPayload.error),
    );
    if (providerError) {
      throw new Error(
        `x-token-exchange-${providerError.replace(/\s+/g, "-").toLowerCase()}`,
      );
    }
...
const fetchAuthenticatedXUser = async ({ accessToken }) => {
  ...
  if (!response.ok) {
    const providerError = toCleanString(
      payload && (payload.title || payload.detail || payload.error),
    );
    if (providerError) {
      throw new Error(
        `x-user-lookup-${providerError.replace(/\s+/g, "-").toLowerCase()}`,
      );
    }
...
exports.xAuthRedirectCallback = onRequest(
  { invoker: "public" },
  async (request, response) => {
    ...
    if (
      expiresAtMs <= 0 ||
      expiresAtMs < nowMs ||
      (createdAtMs > 0 && nowMs - createdAtMs > X_REDIRECT_FLOW_TTL_MS * 2)
    ) {
      await flowRef.set(
        {
          status: "failed",
    ...
    try {
      const callbackUri =
        toCleanString(flowData.callbackUri) ||
        buildXRedirectCallbackUriFromHttpRequest(request);
      ...
      const errorCode = errorCodeRaw
        ? errorCodeRaw.slice(0, 120)
